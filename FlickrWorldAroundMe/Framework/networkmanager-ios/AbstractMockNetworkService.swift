...
// MARK: - Utils Method
                                                                   ...
                                                                   errorBlock:@escaping (_ error: Error, _ validResponse: Bool) -> Void) where ResponseClass: Decodable
    {
        ...
        guard let jsonData: Data = StorageUtils.loadJSON(name: serviceName + ".response", bundle: self.bundle) else
        {
            ...
            errorBlock(NSError.create(message: "Error loading success mock response"), true)
            return
        }
        ...
        guard let jsonObject: ResponseClass = self.objectDecode(jsonData) else
        {
            ...
            errorBlock(NSError.create(message: "Error converting \(ResponseClass.self) object"), true)
            return
        }
        ...
        {
            SystemUtils.mainDelay(after: self.responseDelay.double)
            {
                activityIndicator?.hideActivityIndicator()
        ...
        else
        {
            SystemUtils.mainDelay(after: self.responseDelay.double)
            {
                ...
                errorBlock(NSError.create(message: "Error: loaded failure mock response"), true)
            }
        }
    }
