...
open class AbstractNetworkService
{
    ...
    var reachabilityManagerHost: String = NetworkConstants.ReachabilityDefaultHost
...
// MARK: - Initializers
    ...
    public init(servicePlistName: String, serviceRootKey: String)
    {
        ...
        guard let baseUrl = networkDictionary[NetworkConstants.BaseUrlKey] as? String else
        {
            self.baseAddressString = ""
...
// MARK: - Reachability method
    ...
    public func updateHostForRechability(_ host: String)
    {
        ...
        if (self.reachabilityEnabled == true)
        {
            ...
            SystemUtils.mainDelay(after: 0.5)
            { //[weak self] in
...
// MARK: - Default Header Method
    ...
    {
        self.defaultHeaders += NetworkConstants.DefaultHeaderDictionary
    }
...
// MARK: -  Authentication Header Method
    ...
    {
        let authHeader: HTTPHeaders = [NetworkConstants.AuthHeaderKey : (NetworkConstants.BasicAuthHeaderPreValue + "\(username):\(password)".base64Encoded!)]
    ...
    {
        if (!self.basicAuthHeaderSaved.isEmpty)
        {
    ...
    {
        if let basicAuthValue = self.serviceHeaders[NetworkConstants.AuthHeaderKey]
        {
            self.basicAuthHeaderSaved = [NetworkConstants.AuthHeaderKey : basicAuthValue]
        }
    }
    ...
    {
        let authHeader: HTTPHeaders = [NetworkConstants.AuthHeaderKey : (NetworkConstants.BearerAuthHeaderPreValue + "\(token)")]
        self.addServiceHeader(authHeader)
    }
    ...
    {
        if (!self.bearerAuthHeaderSaved.isEmpty)
        {
    ...
    {
        if let bearerAuthValue = self.serviceHeaders[NetworkConstants.AuthHeaderKey]
        {
            self.bearerAuthHeaderSaved = [NetworkConstants.AuthHeaderKey : bearerAuthValue]
        }
    }
    ...
    {
        self.removeServiceHeaderWithKey(NetworkConstants.AuthHeaderKey)
    }
...
// MARK: - REST Method
                                                                                 ...
                                                                                 errorBlock:@escaping (_ error: Error, _ validResponse: Bool) -> Void)
    {
        ...
        Alamofire.request(restConfiguration.url,
                          ...
                          parameters:parameters,
                          encoding:encoding,
                          headers:restConfiguration.headers).validate(contentType:NetworkConstants.DefaultValidContentTypeArray).responseDecodableObject(queue: self.dispatchQueue,
                                                                                                                                                          keyPath: nil,
                                                                                                                                                          decoder: JSONDecoder(),
                                                                                                                                                          completionHandler:
    ...
    public func canDoRestOperation(errorBlock:@escaping (_ error: Error, _ validResponse: Bool) -> Void) -> Bool
    {
        ...
        if (self.manageReachability() == false)
        {
            ...
            let error = NSError.create(message: "REACHABILITY_RESULT_NETWORK_UNREACHABLE".localized())
                                                       ...
                                                       errorBlock:@escaping (_ error: Error, _ validResponse: Bool) -> Void)
    {
        SystemUtils.mainDelay
        {
            ...
            {
                self.logger.logDebug(responseObject.result.debugDescription)
                let error = NSError.create(message: "Error conversion")
            ...
            {// failure
                ...
                {
                    let error = NSError.create(message: "INTERNAL_ERROR".localized())
                    errorBlock(error, false)
                    return
                }
...
// MARK: - Test related Method
                                            ...
                                            errorBlock:@escaping (_ error: Error, _ validResponse: Bool) -> Void)
    {
        ...
        Alamofire.request(restConfiguration.url,
                          ...
                          parameters:request,
                          encoding:encoding,
                          headers:restConfiguration.headers).validate(contentType:NetworkConstants.DefaultValidContentTypeArray).responseJSON
        { responseObject in
                                   ...
                                   errorBlock:@escaping (_ error: Error, _ validResponse: Bool) -> Void)
    {
        ...
        { responseObject in
            ...
            SystemUtils.mainDelay
            {
                ...
                {
                    self.logger.logDebug("invalid response")
                    let error = NSError.create(message: "Error download image")
                    errorBlock(error, false)
